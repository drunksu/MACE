...
class MACETrainer:
    ...
    def collect_rollouts(self, num_steps: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            rollout_data: Dictionary with keys:
                ...
                - internal_states: list of length num_envs * num_steps, each a dict
                - env_indices: (num_envs * num_steps,) environment indices
        """
        obs_list = []
        action_list = []
        reward_list = []
        ...
        for step in range(num_steps):
            ...
            # Capture internal state for each environment
            for env_i in range(self.config.num_envs):
                internal_state = self.vec_env.get_internal_state(env_i)
    ...
    def compute_group_data(
        ...
        Args:
            ...
            internal_states: list of length batch_size, internal state dicts.
        ...
        Returns:
            ...
            group_log_probs: (batch_size * group_size,) log probabilities under old policy.
            group_advantages: (batch_size * group_size,) group-relative advantages.
            group_internal_states: list of length batch_size * group_size, repeated internal states.
        """
        ...
        # Sample group actions from current policy (old policy at this point)
        with torch.no_grad():
        ...
        # Repeat internal states and env indices
        group_internal_states = []
        for i in range(batch_size):
            for _ in range(group_size):
                group_internal_states.append(internal_states[i])
        group_env_indices = env_indices.repeat_interleave(group_size)
        ...
        return group_states, group_actions, group_log_probs, group_advantages, group_internal_states
    ...
    def compute_loss(
        ...
        Args:
            ...
            actions: (batch_size, action_dim) actions (group actions).
            old_log_probs: (batch_size,) log probabilities under old policy.
            advantages: (batch_size,) advantages.
            internal_states: list of internal states (unused in loss but kept for compatibility).
        ...
        current_log_probs = self.policy_wrapper.compute_log_prob(obs, actions)
    ...
    def update(self, rollout_data: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        internal_states = rollout_data['internal_states']
        env_indices = rollout_data['env_indices']

        # Normalize rewards (optional)
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-8)

        # Compute group data using current policy (old policy before updates)
        group_states, group_actions, group_old_log_probs, group_advantages, group_internal_states = \
            self.compute_group_data(
                obs, actions, rewards, old_log_probs, internal_states, env_indices
            )

        # Convert to tensors
        group_states = group_states.detach()
        group_actions = group_actions.detach()
        group_old_log_probs = group_old_log_probs.detach()
        group_advantages = group_advantages.detach()

        # Update for multiple epochs
        num_samples = group_states.shape[0]
        ...
        for epoch in range(self.config.num_epochs):
            for start in range(0, num_samples, self.config.batch_size):
                ...
                batch_obs = group_states[batch_indices]
                batch_actions = group_actions[batch_indices]
                batch_old_log_probs = group_old_log_probs[batch_indices]
                batch_advantages = group_advantages[batch_indices]
                # internal states not needed for loss
